...
static int createPlayer(struct auplay_st *st, struct auplay_prm *prm)
{
	...
	const SLInterfaceID ids[3] = {SL_IID_BUFFERQUEUE, SL_IID_EFFECTSEND, SL_IID_ANDROIDCONFIGURATION};
	const SLboolean req[3] = {SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE};
	SLAndroidConfigurationItf playerConfig;
	SLuint32 streamType = SL_ANDROID_STREAM_VOICE;
	SLresult r;
	...
	r = (*st->bqPlayerObject)->GetInterface(st->bqPlayerObject,
							...
							&playerConfig);
	if (SL_RESULT_SUCCESS != r) {
		debug("opensles: GetInterface(SL_IID_ANDROIDCONFIGURATION) error");
		return ENODEV;
	}

	r = (*playerConfig)->SetConfiguration(playerConfig,
							SL_ANDROID_KEY_STREAM_TYPE,
							&streamType, sizeof(SLuint32));
	if (SL_RESULT_SUCCESS != r) {
		debug("opensles: SetConfiguration(SL_ANDROID_KEY_STREAM_TYPE) error");
		return ENODEV;
	}
